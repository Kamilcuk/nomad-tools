...
def test_entry_watch2_start():
    job = "test-start"
    mark = "7bc8413c-8619-48bf-a46d-f42727724632"
    exitstatus = 234
    script = f"echo {mark}; sleep 1; exit {exitstatus}"
    try:
        run_entry_watch(f"start -var script={quote(script)} {testjobs[job]}")
        run_entry_watch(f"started {job}", output=[mark])
        cmds = [
            f"stop {job}",
            f"stopped {job}",
            f"--no-follow job {job}",
        ...
        for cmd in cmds:
            run_entry_watch(cmd, check=exitstatus, output=[mark])
    finally:
        run_entry_watch(f"-o nolog stop {job}", check=exitstatus)
    run_entry_watch(f"-o nolog --purge stop {job}", check=exitstatus)


def test_entry_watch2_start2():
    job = "test-start2"
    mark = "7bc8413c-8619-48bf-a46d-f42727724632"
    exitstatus = 234
    script = f"echo {mark}; sleep 1; exit {exitstatus}"
...
def test_entry_watch2_okpurge():
    job = "test-okpurge"
    try:
        # Run nice deployment.
        run_entry_watch(f"start -var ok=true {testjobs[job]}")
        run_entry_watch(f"-x stop {job}")
    finally:
        run_entry_watch(f"-o nolog --debug events -x purge {job}")


def test_entry_watch2_canary():
    """
    ...
    try:
        # Run nice deployment.
        run_entry_watch(f"start -var ok=true  {testjobs[job]}", output=output)
        ...
        run_entry_watch(f"-o nolog -x stop {job}")
    finally:
        run_entry_watch(f"-o nolog -x purge {job}")


def test_entry_watch2_blocked():
    """
...
def test_entry_watch2_maintask():
    """
...
def test_entry_watch2_multiple():
    job = "test-multiple"
    ...
    run_entry_watch(f"--purge run {testjobs[job]}", output=output)


def test_entry_watch2_invalidconfig():
...
def test_entry_watch2_deploymulti():
    ...
    try:
        ...
        run_entry_watch(f" -o none -x purge {job}")


def test_entry_watch2_onestays():
    job = "test-onestays"
    try:
        run_entry_watch(f"-x purge {job}")
        run_entry_watch(
        ...
        run_entry_watch(f"purge {job}", check=False, timeout=20)
    finally:
        run_entry_watch(f"-o none -x purge {job}")
